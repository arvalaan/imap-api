...
class Account {
    ...
    async updateMessage(message, updates, options) {
        await this.loadAccountData(this.account, true);

        options = options || {};

        let timeout = options.timeout;
        if (!timeout && updates && updates.path) {
            // moving many messages for a single account can queue up,
            // so use a longer inter-worker timeout for move operations
            timeout = 5 * 60 * 1000;
        }

        return await this.call({ cmd: 'updateMessage', account: this.account, message, updates, timeout });
    }
    ...
    async bulkMoveMessages(messages, path, options) {
        await this.loadAccountData(this.account, true);

        options = options || {};
        let timeout = options.timeout || 10 * 60 * 1000;

        return await this.call({ cmd: 'bulkMoveMessages', account: this.account, messages, path, timeout });
    }


    async assignLabel(message, label, options) {
        await this.loadAccountData(this.account, true);

        options = options || {};

        return await this.call({ cmd: 'assignLabel', account: this.account, message, label, options });
    }

    async bulkAssignLabel(messages, label, options) {
        await this.loadAccountData(this.account, true);

        options = options || {};
        let timeout = options.timeout || 10 * 60 * 1000;

        return await this.call({ cmd: 'bulkAssignLabel', account: this.account, messages, label, options, timeout });
    }
    ...
    async ensureLabel(label, options) {
        ...
        return await this.call({ cmd: 'ensureLabel', account: this.account, label, options });
    }

    async removeLabel(message, label, options) {
        await this.loadAccountData(this.account, true);

        options = options || {};

        return await this.call({ cmd: 'removeLabel', account: this.account, message, label, options });
    }

    async bulkRemoveLabel(messages, label, options) {
        await this.loadAccountData(this.account, true);

        options = options || {};
        let timeout = options.timeout || 10 * 60 * 1000;

        return await this.call({ cmd: 'bulkRemoveLabel', account: this.account, messages, label, options, timeout });
    }
