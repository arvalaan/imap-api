...
class Connection {
    ...
    async start() {
        ...
        try {
            ...
            if (err.authenticationFailed) {
                this.logger.error({ msg: 'Failed to authenticate', err });
                this.notify(false, 'authenticationError', {
                ...
                this.logger.error({ msg: 'Failed to connect', err });
    ...
    resolveLabelDestination(label, options) {
        let labelName = ((label || '') + '').trim();
        if (!labelName) {
            let err = new Error('Label name must not be empty');
            err.code = 'InvalidLabelName';
            err.statusCode = 400;
            throw err;
        }

        if (/[\\/]/.test(labelName)) {
            let err = new Error('Label name must not contain path separators');
            err.code = 'LabelPathNotAllowed';
            err.statusCode = 400;
            throw err;
        }

        options = options || {};
        let prefix = ((options.prefix || 'Labels') + '').trim() || 'Labels';
        let destination = normalizePath(`${prefix}/${labelName}`);
    ...
    async assignLabel(id, label, options) {
        this.checkIMAPConnection();

        let { label: labelName, destination } = this.resolveLabelDestination(label, options);
        ...
        let sourcePath = normalizePath(message.path);
        ...
        if (!this.mailboxes.has(sourcePath)) {
            return {
                id,
                assigned: false,
                label: labelName,
        ...
        let mailbox = this.mailboxes.get(sourcePath);
        let moved = await mailbox.moveMessages([{ uid: message.uid }], destination);
        let messageId = id;
        if (moved.uidMap && moved.uidMap.has(message.uid)) {
            messageId = await this.packUid(moved.destination, moved.uidMap.get(message.uid));
        }
    ...
    async bulkAssignLabel(ids, label, options) {
        this.checkIMAPConnection();
        ...
        for (let id of ids) {
            try {
                ...
                if (!message || !message.path) {
                    resolved.push({ id, assigned: false, error: 'Message not found' });
                    continue;
                }

                let sourcePath = normalizePath(message.path);
                if (sourcePath === destination) {
                    resolved.push({ id, assigned: true, noop: true, destination, messageId: id });
                    continue;
                }

                resolved.push({ id, assigned: false, path: message.path, uid: message.uid, destination });
            } catch (err) {
                resolved.push({ id, assigned: false, error: err.message });
            }
        }

        let byPath = new Map();
        for (let item of resolved) {
            if (item.error || !item.path || !item.uid) {
                continue;
            }
        ...
        for (let [sourcePath, items] of byPath) {
            if (!this.mailboxes.has(sourcePath)) {
                items.forEach(item => {
                    item.error = 'Source mailbox not found';
                });
                continue;
            }

            let mailbox = this.mailboxes.get(sourcePath);
            let moved = await mailbox.moveMessages(items, destination);
            items.forEach(item => {
                item.destination = moved.destination;
                item.assigned = true;
                if (moved.uidMap && moved.uidMap.has(item.uid)) {
                    item.messageId = this.packUid(moved.destination, moved.uidMap.get(item.uid));
                }
            });

            for (let item of items) {
                if (item.messageId && typeof item.messageId.then === 'function') {
                    item.messageId = await item.messageId;
                }
                if (!item.messageId) {
        ...
        let messages = resolved.map(item => ({
            id: item.id,
        ...
        return {
            assigned: messages.filter(message => message.assigned).length,
            failed: messages.filter(message => !message.assigned).length,
            destination,
            label: labelName,
            messages
        };
    ...
    async ensureLabel(label, options) {
        this.checkIMAPConnection();

        let { destination } = this.resolveLabelDestination(label, options);

        let exists = this.mailboxes.has(normalizePath(destination));
        if (exists) {
            return {
                path: destination,
                created: false,
        ...
        let result = await this.createMailbox(destination);

        return {
            path: (result && result.path) || destination,
            created: !!(result && result.created),
            existing: !result || !result.created
        };
    }

    async removeLabel(id, label, options) {
        this.checkIMAPConnection();
        ...
        let sourcePath = normalizePath(message.path);
        let labelPath = normalizePath(destination);

        if (!this.mailboxes.has(labelPath)) {
            return {
                id,
                removed: true,
                noop: true,
        ...
        let labelMailbox = this.mailboxes.get(labelPath);
        let uidList = [];

        if (sourcePath === labelPath) {
            uidList = [message.uid];
        } else {
            if (!this.mailboxes.has(sourcePath)) {
                return {
            ...
            let sourceMailbox = this.mailboxes.get(sourcePath);
            let messageData = await sourceMailbox.getMessage(message, { headers: false, textType: false });
            if (!messageData || !messageData.emailId) {
                return {
                    id,
                    removed: false,
                    label: labelName,
                    source: sourcePath,
                    destination,
                    error: 'EmailId not available for label removal'
                };
            }

            uidList = await labelMailbox.findMessageUidsByEmailId(messageData.emailId);
        }
        ...
        await labelMailbox.deleteMessagesByUid(uidList);
        ...
        return {
            ...
            source: sourcePath,
            destination,
            matches: uidList.length
        };
    }

    async bulkRemoveLabel(ids, label, options) {
        this.checkIMAPConnection();
        ...
        for (let id of ids) {
            try {
                messages.push(await this.removeLabel(id, labelName, options));
            } catch (err) {
                messages.push({
                    id,
                    removed: false,
        ...
        return {
            removed: messages.filter(message => message.removed).length,
            failed: messages.filter(message => !message.removed).length,
            destination,
            label: labelName,
            messages
        };
    ...
    async bulkMoveMessages(ids, path) {
        this.checkIMAPConnection();

        ids = Array.isArray(ids) ? ids : [];
        let destinationPath = normalizePath(path || '');

        if (!ids.length || !destinationPath) {
            return { moved: 0, messages: [] };
        }
        ...
        for (let id of ids) {
            try {
                ...
                let message = await this.unpackUid(buf.slice(0, 8));
                if (!message || !message.path) {
                    resolved.push({ id, moved: false, error: 'Message not found' });
                    continue;
                }
                resolved.push({ id, moved: false, path: message.path, uid: message.uid });
            } catch (err) {
                resolved.push({ id, moved: false, error: err.message });
            }
        }
        ...
        for (let [sourcePath, items] of byPath) {
            ...
            let mailbox = this.mailboxes.get(sourcePath);
            if (normalizePath(mailbox.path) === destinationPath) {
                items.forEach(item => {
                    item.moved = true;
            ...
            let moved = await mailbox.moveMessages(items, destinationPath);
            items.forEach(item => {
                item.destination = moved.destination;
                item.moved = true;
                if (moved.uidMap && moved.uidMap.has(item.uid)) {
        ...
        return {
            moved: messages.filter(message => message.moved).length,
            failed: messages.filter(message => !message.moved).length,
            messages
        };
    }
