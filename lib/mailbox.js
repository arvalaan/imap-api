...
class Mailbox {
            ...
            if (options.textType && messageInfo.text && messageInfo.text.id) {
                ...
                if (textParts && textParts.length) {
                    ...
                    if (textParts && textParts.length) {
                        ...
                        if ((!['plain', 'html'].includes(options.textType) && messageInfo.inReplyTo) || (messageInfo.headers && messageInfo.references)) {
                            ...
                            try {
                                ...
                                if (!plaintext && messageInfo.text.html && messageInfo.text.html.length < MAX_HTML_PARSE_LENGTH) {
                                    plaintext = htmlToText.fromString(messageInfo.text.html);
                                }
    ...
    async updateMessage(message, updates) {
        ...
        try {
            ...
            if (updates.path && normalizePath(updates.path) !== normalizePath(this.path)) {
                let moved = await this.imapClient.messageMove(message.uid, updates.path, { uid: true });
                if (moved) {
                    result.moved = {
                        destination: moved.destination
                    };
                    if (moved.uidMap && moved.uidMap.has(message.uid)) {
                        result.moved.messageId = await this.connection.packUid(moved.destination, moved.uidMap.get(message.uid));
                    }
                }
            }
    ...
    async moveMessages(messages, destinationPath) {
        ...
        try {
            if (!messages.length || !destinationPath || normalizePath(destinationPath) === normalizePath(this.path)) {
                return {
                    moved: 0,
                    destination: destinationPath || this.path,
                    uidMap: new Map()
                };
            }

            let uids = messages.map(message => message.uid).filter(uid => uid && !isNaN(uid));
            if (!uids.length) {
                return {
                    moved: 0,
            ...
            let moveResponse = await this.imapClient.messageMove(uids, destinationPath, { uid: true });
            return {
                moved: uids.length,
                destination: (moveResponse && moveResponse.destination) || destinationPath,
                uidMap: (moveResponse && moveResponse.uidMap) || new Map()
            };
        } finally {
            lock.release();
            this.connection.onTaskCompleted();
    ...
    async findMessageUidsByEmailId(emailId) {
        if (!emailId) {
            return [];
        }
        ...
        try {
            let uidList = await this.imapClient.search({ emailId }, { uid: true });
            return Array.isArray(uidList) ? uidList : [];
        } finally {
            lock.release();
            this.connection.onTaskCompleted();
        }
    ...
    async deleteMessagesByUid(uids) {
        uids = (Array.isArray(uids) ? uids : [uids]).filter(uid => uid && !isNaN(uid));
        ...
        try {
            return await this.imapClient.messageDelete(uids, { uid: true });
        } finally {
            lock.release();
            this.connection.onTaskCompleted();
        }
